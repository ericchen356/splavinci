...
export type AssetSet = {
  ...
  /**
   * Transform for the collider, when it does not arrive in world space.
   *
   * Marble returns the collider in the same OpenCV frame as the splat and at
   * the same pre-metric scale - its raw extent came out 7.6 x 1.74 x 6.24 m,
   * where 1.74 m is obviously not a room height. Applying the splat's own
   * metric_scale_factor gives 14.7 x 3.4 x 12.1 m and puts the floor at y = 0,
   * so the two do belong in one frame. Their docs only describe
   * semantics_metadata against the SPZ export, so this was confirmed by
   * measuring the mesh rather than read from a spec.
   *
   * Omit for a collider already authored in world space.
   */
  colliderPlacement?: SplatPlacement;
...
export const ASSET_SETS: Record<string, AssetSet> = {
  ...
  'maple-street': {
    ...
    label: 'Maple Street',
    description:
      'Generated by Marble from a blueprint description and three interior ' +
      'photos. Real authored collider - 209k triangles, not derived.',
    splat: '/generated/maple-street/room.spz',
    splatFallback: null,
    collider: '/generated/maple-street/collider.glb',
    objects: '/generated/maple-street/objects.json',
    // Marble exports in the OpenCV frame at pre-metric scale, so both the
    // splat and the collider need the same turn, scale and lift.
    placement: { position: [0, 1.229492, 0], rotation: [Math.PI, 0, 0], scale: 1.9397132 },
    colliderPlacement: { position: [0, 1.229492, 0], rotation: [Math.PI, 0, 0], scale: 1.9397132 },
    approxSplats: 500_000,
  },
  hobbiton: {
    id: 'hobbiton',
    label: 'Hobbiton',
